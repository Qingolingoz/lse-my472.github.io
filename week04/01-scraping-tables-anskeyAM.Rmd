...
library(ggplot2)
#Melt (data_pop[, 1:3], states that you only want the first three columns going by year
...
=======
#go into view developer, view source
>>>>>>> Stashed changes
html <- read_html(url)
...
```{r}
#. before class indicates the css
...
=======
```{r}
#Remove things with square brackets, \\ helps conver square brackets to regular expression. So 
data_pop$city_name <- gsub('\\[.*\\]', '', data_pop$City)
data_pop$population <- data_pop[,"2018estimate"]
#Remove commas and making things numeric
data_pop$population <- as.numeric(gsub(",", "", data_pop$population))
data_pop$rank <- data_pop[,"2018rank"]
#Write over data_pop with these three columns
